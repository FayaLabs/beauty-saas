...
| google-calendar (from plugin-agenda) | SDK-owned connector, migrations and canonical Edge templates; app deployment copy is parity-checked; signed OAuth, tenant-bound delivery, webhook dedupe, Watch renewal and reconciliation |
...
- Google Calendar schema, connector and canonical Edge templates originate in `plugin-agenda`; the app owns only the Supabase deployment copy and pool-specific operational runbook.
