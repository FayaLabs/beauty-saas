...
function tenantId(): string {
  ...
  if (!value) throw new Error('Selecione um salão antes de conectar o Google Calendar')
  return value
}
  ...
  return {
    ...
    async getSyncLog(): Promise<CalendarSyncLogEntry[]> {
      ...
      return (data ?? []).map((row: any) => ({
        id: row.id, direction: row.direction, trigger: row.trigger ?? undefined,
        status: row.status, fetched: row.fetched ?? 0, written: row.written ?? 0,
