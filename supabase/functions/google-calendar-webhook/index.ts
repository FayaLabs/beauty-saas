...
import { fetchWithRetry } from '../_shared/google-calendar-runtime.js'
...
async function pullChangedEvents(db: any, deliveryId: string, tenantId: string, channelRef: string) {
  ...
  try {
    const deliverySecret = requiredEnv('GCAL_OUTBOUND_SECRET')
    const syncUrl =
      env('GCAL_SYNC_FUNCTION_URL') ||
      `${requiredEnv('SUPABASE_URL')}/functions/v1/google-calendar-sync`
    const response = await fetchWithRetry(fetch, syncUrl, {
      method: 'POST',
      headers: {
    ...
    console.error(JSON.stringify({
      event: 'gcal.webhook.pull_failed',
      deliveryId,
      tenantId,
      channelRef,
